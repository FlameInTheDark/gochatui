	...
	<header class="sticky top-0 z-50 border-b border-white/10 bg-ink-900/70 backdrop-blur">
		<div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 sm:px-8">
			...
			<div class="flex items-center gap-2">
				<div
					class="relative"
					bind:this={languageMenuContainer}
					onfocusout={handleLanguageMenuFocusOut}
				>
					<button
						type="button"
						class="group inline-flex h-12 min-w-[160px] items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/5 px-4 text-left text-sm text-zinc-200 transition outline-none focus:border-violet-400/70 focus:ring-2 focus:ring-violet-500/50"
					...
					>
						<span class="flex flex-1 flex-col">
							<span
								class="text-[10px] tracking-[0.2em] text-zinc-400/80 uppercase transition-colors group-hover:text-zinc-300"
							>
								{m.language()}
							</span>
						...
						>
							{#each languageOptions as option (option.code)}
								<button
									...
									class={`flex w-full items-center justify-between gap-2 rounded-md px-3 py-2 text-sm transition hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-violet-500/50 focus-visible:outline-none ${
										option.code === $locale ? 'bg-violet-500/20 text-white' : 'text-zinc-200'
									}`}
									onclick={() => selectLocale(option.code)}
								>
									<span>{option.label()}</span>
									{#if option.code === $locale}
										<svg class="h-3 w-3 text-violet-300" viewBox="0 0 12 10" aria-hidden="true">
											<path
			...
			<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				<div class="h-full rounded-2xl border border-white/10 bg-ink-900/70 p-5">
					...
					>
						...
						<span class="text-sm font-semibold text-zinc-100"
							>{m.landing_feature_channels_title()}</span
						>
					</div>
				...
				<div class="h-full rounded-2xl border border-white/10 bg-ink-900/70 p-5">
					...
					>
						...
						<span class="text-sm font-semibold text-zinc-100"
							>{m.landing_feature_search_title()}</span
						>
					</div>
					<p class="text-sm leading-relaxed text-zinc-400">
				...
				<div class="h-full rounded-2xl border border-white/10 bg-ink-900/70 p-5">
					...
					>
						...
						<span class="text-sm font-semibold text-zinc-100"
							>{m.landing_feature_roles_title()}</span
						>
					</div>
					<p class="text-sm leading-relaxed text-zinc-400">
				...
				<div class="h-full rounded-2xl border border-white/10 bg-ink-900/70 p-5">
					...
					>
						...
						<span class="text-sm font-semibold text-zinc-100"
							>{m.landing_feature_notifications_title()}</span
						>
						<span
				...
				<div class="h-full rounded-2xl border border-white/10 bg-ink-900/70 p-5">
					...
					>
						...
						<span class="text-sm font-semibold text-zinc-100"
							>{m.landing_feature_secure_title()}</span
						>
						<span
							class="ml-2 rounded-md border border-violet-500/30 bg-violet-500/10 px-1.5 py-0.5 text-[10px] text-violet-200"
							>{m.landing_feature_badge_coming_soon()}</span
				...
				<div class="h-full rounded-2xl border border-white/10 bg-ink-900/70 p-5">
					...
					>
						...
						<span class="text-sm font-semibold text-zinc-100"
							>{m.landing_feature_voice_title()}</span
						>
						<span
							class="ml-2 rounded-md border border-violet-500/30 bg-violet-500/10 px-1.5 py-0.5 text-[10px] text-violet-200"
							>{m.landing_feature_badge_coming_soon()}</span
		...
		>
			...
			<nav class="flex items-center gap-4 text-sm text-zinc-400">
				...
				<a
					...
					target="_blank"
					rel="noopener">{m.landing_footer_status_link()}</a
				>
			</nav>
		</div>
	</footer>

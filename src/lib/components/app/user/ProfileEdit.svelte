<script lang="ts">
	...
	$: avatarUrl = previewUrl ?? resolveExistingAvatar($me);
	...
	function handleZoomInput(event: Event) {
		...
		let nextX = cropSize / 2 - centerX * nextScale;
		let nextY = cropSize / 2 - centerY * nextScale;
...
<div class="space-y-4">
	<div class="panel space-y-4 p-4">
		<div class="flex items-center justify-between gap-3">
			...
			>
				{#if avatarUrl}
					<img alt="Avatar preview" class="h-full w-full object-cover" src={avatarUrl} />
				{:else}
					<span class="text-lg font-semibold text-[var(--muted)]">{avatarInitial()}</span>
				{/if}
			</div>
		</div>
		...
		{#if imageObjectUrl}
			<div class="flex flex-col gap-3 md:flex-row md:items-start">
				...
				<div class="flex flex-1 flex-col gap-3">
					...
					<button
						class="self-start rounded-md border border-[var(--stroke)] px-3 py-1 text-sm"
						onclick={resetAvatar}
					>

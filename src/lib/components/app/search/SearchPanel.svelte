<script lang="ts">
	...
	$effect(() => {
		pendingHasNormalized = pendingFilter === 'has' ? normalizeHasValue(pendingValue) : null;
	});
	...
	async function doSearch() {
		...
		const hasFilters = Boolean(authorFilter) || mentionFilters.length > 0 || hasSelected.length > 0;
...
{#if $searchOpen}
	...
	>
		...
		<div class="absolute z-10" style={`left:${posX}px; top:${posY}px`}>
			<div class="relative">
				<div
					class="pointer-events-none absolute inset-0 z-0 rounded-xl bg-[var(--panel)]/30 backdrop-blur-sm"
				></div>
				<div
					bind:this={panelEl}
						...
						>
							...
							>
								...
								{#if authorFilter}
									...
									>
										...
										<button
											class="rounded-full bg-transparent p-1 text-[var(--muted)] hover:bg-[var(--panel-strong)] focus-visible:ring-2 focus-visible:ring-[var(--brand)]/40 focus-visible:outline-none"
											type="button"
								...
								{#if pendingFilter === 'from' || pendingFilter === 'mentions'}
									<div
										class="flex items-center gap-2 rounded-md border border-[var(--stroke)] bg-[var(--panel)] px-2 py-1 text-xs text-[var(--fg)]"
									>
										<span class="font-semibold text-[var(--muted)]">
											{pendingFilter === 'from'
												? m.search_filter_from()
										...
										<input
											...
											class="w-40 border-none bg-transparent text-sm text-[var(--fg)] placeholder:text-[var(--muted)] focus:outline-none focus-visible:ring-0 focus-visible:outline-none"
										...
										<button
											class="rounded-md bg-[var(--brand)] px-2 py-1 text-xs font-semibold text-[var(--bg)] transition hover:bg-[var(--brand)]/90 focus-visible:ring-2 focus-visible:ring-[var(--brand)]/40 focus-visible:outline-none disabled:opacity-50"
											type="button"
											onclick={(event) => {
												event.stopPropagation();
												applyPendingFilter();
								...
								{:else if pendingFilter === 'has'}
									<div
										class="flex flex-wrap items-center gap-2 rounded-md border border-[var(--stroke)] bg-[var(--panel)] px-2 py-1 text-xs text-[var(--fg)]"
									>
										<span class="font-semibold text-[var(--muted)]">{m.search_filter_has()}</span>
										<input
											bind:this={promptInputEl}
											class="w-32 border-none bg-transparent text-sm text-[var(--fg)] placeholder:text-[var(--muted)] focus:outline-none focus-visible:ring-0 focus-visible:outline-none"
										...
										<div class="flex flex-wrap gap-1">
											{#each hasOptions as option}
												<button
													class={`rounded-full border px-3 py-1 text-xs transition focus-visible:ring-2 focus-visible:ring-[var(--brand)]/40 focus-visible:outline-none ${
														option.value === pendingHasNormalized ||
								...
								<input
									...
									class="min-w-[6rem] flex-1 border-none bg-transparent text-sm text-[var(--fg)] placeholder:text-[var(--muted)] focus:outline-none focus-visible:ring-0 focus-visible:outline-none"
								...
								<button
									...
									class="ml-2 shrink-0 rounded-md border border-[var(--stroke)] px-2 py-1.5 text-sm text-[var(--muted)] transition hover:text-[var(--fg)] focus-visible:ring-2 focus-visible:ring-[var(--brand)]/40 focus-visible:outline-none"
									type="button"
								...
								<button
									class="ml-2 shrink-0 rounded-md bg-[var(--brand)] px-3 py-1.5 text-sm font-semibold text-[var(--bg)] transition hover:bg-[var(--brand)]/90 focus-visible:ring-2 focus-visible:ring-[var(--brand)]/40 focus-visible:outline-none disabled:opacity-50"
									type="button"
									onclick={() => {
										showKeywordHelp = false;
										page = 0;
							...
							{#if showKeywordHelp}
								<div
									...
									class="absolute top-full right-0 z-30 mt-2 w-64 rounded-lg border border-[var(--stroke)] bg-[var(--panel-strong)] p-4 text-xs text-[var(--fg)] shadow-xl"
								>
										...
										<div class="min-w-0 flex-1">
											<div class="flex flex-wrap items-baseline gap-x-2 gap-y-1">
												...
												<div
													...
													use:tooltip={() => formatMsgFull(message)}
												>

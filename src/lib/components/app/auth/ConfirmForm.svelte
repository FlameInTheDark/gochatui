<script lang="ts">
  ...
  async function onSubmit() {
    ...
    try {
      ...
      error = e?.response?.data?.message ?? e?.message ?? 'Confirmation failed';
    } finally {
      loading = false;
    }
  }
...
<form class="space-y-3" on:submit|preventDefault={onSubmit}>
  <div class="text-lg font-semibold">Confirm account</div>

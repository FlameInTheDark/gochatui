		...
		<div class="panel w-full max-w-md p-6">
			{#if mode === 'login'}
				...
				<div class="mt-3 flex justify-between text-sm text-[var(--muted)]">
					...
					<button class="underline" type="button" onclick={() => (mode = 'recovery')}>
						{m.auth_forgot()}
					</button>
				</div>
				<div class="mt-2 space-x-1 text-xs text-[var(--muted)]">
					<span>Or</span>
					<button class="underline" type="button" onclick={() => (mode = 'confirm')}>
						{m.auth_confirm_word()}
					</button>
					<span>/</span>
					<button class="underline" type="button" onclick={() => (mode = 'reset')}>
						{m.auth_reset_word()}
					</button>
			...
			{:else if mode === 'register'}
				...
				<div class="mt-3 text-sm text-[var(--muted)]">
					<button class="underline" type="button" onclick={() => (mode = 'login')}>
			...
			{:else if mode === 'register-success'}
				<div class="space-y-6 text-center">
					<div
						class="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-[var(--brand)] text-[var(--brand)]"
					>
			...
			{:else if mode === 'confirm'}
				<ConfirmForm defaults={confirmDefaults} on:success={() => (mode = 'login')} />
				<div class="mt-3 text-sm text-[var(--muted)]">

...
<div class="flex gap-3 px-3 py-2 opacity-90" data-pending-message>
	<div
		class="h-10 w-10 flex-shrink-0 overflow-hidden rounded-full border border-[var(--stroke)] bg-[var(--panel)]"
	>
		{#if avatarUrl()}
			<img src={avatarUrl()!} alt={userDisplayName()} class="h-full w-full object-cover" />
		{:else}
			<div
				class="grid h-full w-full place-items-center text-sm font-semibold text-[var(--brand)] uppercase"
			>
	...
	<div class="min-w-0 flex-1">
		...
		{#if message.content}
			<div class="mt-1 text-sm whitespace-pre-wrap text-[var(--fg)]">{message.content}</div>
		{/if}
				...
				{#each message.attachments as attachment (attachment.localId)}
					...
					>
						...
						{#if attachment.status === 'error' && attachment.error}
							<div
								class="absolute inset-x-0 bottom-1 flex items-center gap-1 px-2 text-[10px] text-red-400"
							>

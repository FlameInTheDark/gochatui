<script lang="ts">
	...
	function findGuildLayoutEntry(
		layout: GuildLayoutItem[],
		guildId: string
	): GuildLayoutGuild | null {
			...
			for (const [guildId, states] of updatesByGuild) {
				if (guildId === '@me') {
					...
					for (const state of states) {
						const idx = nextDmChannels.findIndex((entry) => entry.channelId === state.channelId);
						if (idx === -1) continue;
	...
	async function runAck(guildId: string, channelId: string, messageId: string, key: string) {
		ackTimer = null;
		scheduledAckKey = null;
		ackInFlightKey = key;
		let success = false;
	...
	$effect(() => {
		...
		const lastReadMarker =
			persisted?.lastReadMessageId ??
			lookup?.[gid]?.[cid]?.lastReadMessageId ??
			activeChannelReadMarker;
	...
	$effect(() => {
		...
		if (initializedChannelKey === key && (initialLoaded || initializingChannelKey === key)) {
			return;
		}
		const list = $channelsByGuild[gid] ?? [];
	...
	{#each messages as m, i (m.id)}
		...
		{#if showNewSeparator}
			<div class="my-4 flex items-center gap-3 pl-3" data-new-message-separator>
				...
				<span class="text-xs font-semibold tracking-wide text-red-500 uppercase">

...
function normalizeAvatarValue(value: unknown, visited = new Set<unknown>()): string | null {
        ...
        if (typeof value === 'object') {
                ...
                if ('avatar' in record) {
                        const resolved = normalizeAvatarValue(record.avatar, visited);
                        if (resolved) {
                                return resolved;
                        }
                }
                if ('avatarUrl' in record) {
                        const resolved = normalizeAvatarValue(record.avatarUrl, visited);
                        if (resolved) {
                                return resolved;
                        }
                }
                if ('avatar_url' in record) {
                        const resolved = normalizeAvatarValue(record.avatar_url, visited);
                        if (resolved) {
                                return resolved;
                        }
                }

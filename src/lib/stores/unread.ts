...
import {
	...
	type GuildChannelReadStateLookup
...
export function acknowledgeChannelRead(guildId: unknown, channelId: unknown): void {
		...
		if (Array.isArray(payload)) {
			for (const entry of payload) {
				...
				if (entry && typeof entry === 'object') {
					...
					const channelId =
						normalizeId(
							obj['channel_id'] ?? obj['channelId'] ?? (obj['channel'] as any)?.id ?? obj['id']
					...
					const lastMessageId = normalizeId(
						obj['last_message_id'] ??
							...
							obj['latestMessageId'] ??
							obj['messageId'] ??
							(Array.isArray(obj['value']) ? obj['value'][1] : (obj['value'] as unknown)) ??
							obj['value'] ??
							obj['last']
					);
			...
			for (const entry of value) {
				...
				if (Array.isArray(entry)) {
					if (entry.length >= 3) {
						...
						recordEntry(normalizeId(guildKey), normalizeId(channelKey), normalizeId(messageValue));
					}
					continue;
				}
